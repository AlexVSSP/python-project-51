...
import tempfile
from page_loader import download, download_image, make_dir_name
...
#             download(url, tmpdir)
#         # path = f'{tmpdir}/page-loader-hexlet-repl-co-.html'
#         path = os.path.join(tmpdir, 'page-loader-hexlet-repl-co-.html')
#         assert os.path.exists(path)
#         with open(path, 'rb') as f:
#         # print(f.read().decode("utf-8"))
#         # print(nodejs_page_content.decode("utf-8"))
...
def test_create_dir():
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        with requests_mock.Mocker() as m:
            m.get(url)
            download(url, tmpdir)
        # correct_dir = f'{tmpdir}/www-gov-uk_files'
...
@pytest.fixture
def nodejs_page_content():
    with open("tests/fixtures/page-loader-hexlet-repl-co-assets-professions-nodejs.png", 'rb') as f:
        return f.read()
